...
Działu Produkcyjnego. Oznacza to, że rozkład odpowiedzi na PYT_1 nie
...
chi-kwadrat. Zwraca ona statystykę testu chi-kwadrat, stopnie swobody,
p-value. Na tej podstawie można ocenić, czy zmienne są niezależne.
...
lub "NIE". Kolor czerwony słupka określa, że jest mniej przypadków niż
...
$p_{11} = 1/40$, $p_{12} = 3/40$, $p_{21} = 19/40$, $p_{22} = 17/40$.
Symulacje wykonano dla $n =50$, $n=100$ oraz $n=1000$.\\
...
niemal zawsze wykrywają zależność (moc ≈ 1). Test Fishera okazuje się
...
($p>\alpha$). Oznacza to, że test $G^{2}$ nie wykazał istotnego
...
przy danej liczności próby oraz przyjętym poziomie istotności. \newpage
...
prawdopodobieństw: - $\pi_1=P(Zdarzenie|Grupa1)$, -
$\pi_2=P(Zdarzenie|Grupa2)$. gdzie $\pi_1, \pi_2 \in (0,1)$.
...
szans (OR). Intuicyjnie wynika to z faktu, że OR „przesadza” efekt
